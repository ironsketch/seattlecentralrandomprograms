...
public class Square {
	...
	public void rotate(int row1, int col1){
		int coli = col;
		int rowi = row;
		row = row1 + (coli - col1);
		col = col1 + (row1 - rowi);
	...
	public boolean canRotate(int row1, int col1){
		int rowi = row;
		int coli = col;
		
		if (coli == (Grid.WIDTH - 1) || coli == 0 || rowi == (Grid.HEIGHT - 1) || grid.isSet(( row1 + (coli - col1)),( col1 + (row1 - rowi)))){
			return false;
		}
		return true;
	}
